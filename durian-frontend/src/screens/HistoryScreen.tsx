...
import { Clock, Trash2, ChevronRight, Globe, User as UserIcon, ShieldCheck } from 'lucide-react-native';
...
export function HistoryScreen() {
  ...
  const { scans, globalScans, isLoaded, loadHistory, deleteScan, simulateGlobalActivity } = useHistoryStore();
  const { user } = useAuthStore();
  const [activeTab, setActiveTab] = useState<'personal' | 'global'>(user?.role === 'admin' ? 'global' : 'personal');

  useEffect(() => {
    if (!isLoaded) loadHistory();
    const cleanup = simulateGlobalActivity();
    return cleanup;
  }, [isLoaded, loadHistory, simulateGlobalActivity]);
  ...
  const renderItem = ({ item, index }: { item: ScanRecord; index: number }) => (
    <Animated.View style={[styles.historyItem, Shadows.card]}>
      <View style={styles.itemIconWrapper}>
        <Text style={styles.itemIcon}>{VARIETY_EMOJI[item.variety] ?? '🍈'}</Text>
      </View>
      ...
      {activeTab === 'personal' && (
        <TouchableOpacity onPress={() => handleDelete(item.id)} style={styles.deleteBtn} hitSlop={12}>
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <UserIcon size={16} color={activeTab === 'personal' ? colors.white : colors.textMuted} />
          <Text style={[styles.tabText, activeTab === 'personal' && styles.activeTabText]}>My Scans</Text>
        </TouchableOpacity>
        ...
        >
          <Globe size={16} color={activeTab === 'global' ? colors.white : colors.textMuted} />
          <Text style={[styles.tabText, activeTab === 'global' && styles.activeTabText]}>Community</Text>
          {user?.role === 'admin' && <ShieldCheck size={12} color={activeTab === 'global' ? colors.white : colors.primary} />}
        </TouchableOpacity>
      </View>
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.pageSubtitle}>
              {activeTab === 'personal' 
                ...
                : 'Real-time scans from 10 most recent users.'}
            </Text>
          </View>
        )}
        ListEmptyComponent={() => (
          <View style={styles.emptyState}>
            <Text style={styles.emptyEmoji}>{activeTab === 'personal' ? '🍈' : '🌍'}</Text>
            <Text style={styles.emptyTitle}>Nothing here yet!</Text>
            <Text style={styles.emptySubtitle}>
              {activeTab === 'personal' ? 'Start recording to build your history.' : 'Connecting to community feed...'}
            </Text>
          </View>
        )}
...
const getStyles = (colors: any) => StyleSheet.create({
  ...
  itemIconWrapper: {
    ...
    height: 36,
    borderRadius: 18,
    backgroundColor: colors.accentYellow + (colors.bgLight === '#12140F' ? '33' : '66'),
    alignItems: 'center',
    justifyContent: 'center',
  },
  ...
  userBadgeText: { ...Typography.caption, fontSize: 9, color: colors.primaryDark, fontWeight: '800' },
