...
const API_BASE_URL = 'https://api.durly.app'; // Replace with your actual endpoint

// Mock database of durian varieties for demo
const DURIAN_VARIETIES = ['Musang King', 'D24 Sultan', 'Black Thorn', 'Golden Phoenix', 'Red Prawn'];
...
export async function mockAnalyzeAudio(_audioUri: string): Promise<AIResult> {
  ...
  const ripenessOptions: RipenessType[] = ['ripe', 'ripe', 'ripe', 'under_ripe', 'over_ripe'];
  const ripeness = ripenessOptions[Math.floor(Math.random() * ripenessOptions.length)];
  const confidence = ripeness === 'ripe' ? 0.85 + Math.random() * 0.15 : 0.65 + Math.random() * 0.25;
  const variety = DURIAN_VARIETIES[Math.floor(Math.random() * DURIAN_VARIETIES.length)];
